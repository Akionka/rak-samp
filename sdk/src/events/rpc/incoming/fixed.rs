...
pub const SET_VEHICLE_POSITION: IncomingRpc<VehiclePosition> =
    IncomingRpc::new(159, decode_vehicle_position, encode_vehicle_position);
/// The `onSetVehicleAngle` descriptor (`vehicle_id`, then angle).
pub const SET_VEHICLE_ANGLE: IncomingRpc<VehicleAngle> =
    IncomingRpc::new(160, decode_vehicle_angle, encode_vehicle_angle);
/// The `onSetVehicleHealth` descriptor.
pub const SET_VEHICLE_HEALTH: IncomingRpc<VehicleHealth> =
    IncomingRpc::new(147, decode_vehicle_health, encode_vehicle_health);
/// The `onResetPlayerMoney` descriptor.
pub const RESET_PLAYER_MONEY: IncomingRpc<()> = IncomingRpc::new(20, decode_empty, encode_empty);
/// The `onResetPlayerWeapons` descriptor.
pub const RESET_PLAYER_WEAPONS: IncomingRpc<()> = IncomingRpc::new(21, decode_empty, encode_empty);
/// The `onCancelEdit` descriptor.
pub const CANCEL_EDIT: IncomingRpc<()> = IncomingRpc::new(28, decode_empty, encode_empty);
/// The `onSetToggleClock` descriptor.
pub const SET_TOGGLE_CLOCK: IncomingRpc<bool> = IncomingRpc::new(30, decode_bool8, encode_bool8);
/// The `onSetPlayerDrunk` descriptor.
pub const SET_PLAYER_DRUNK: IncomingRpc<i32> = IncomingRpc::new(35, decode_i32, encode_i32);
/// The `onSetRaceCheckpoint` descriptor.
pub const SET_RACE_CHECKPOINT: IncomingRpc<RaceCheckpoint> =
    IncomingRpc::new(38, decode_race_checkpoint, encode_race_checkpoint);
/// The `onPlayAudioStream` descriptor.
pub const PLAY_AUDIO_STREAM: IncomingRpc<AudioStream> =
    IncomingRpc::new(41, decode_audio_stream, encode_audio_stream);
/// The `onSetObjectPosition` descriptor.
pub const SET_OBJECT_POSITION: IncomingRpc<ObjectPosition> =
    IncomingRpc::new(45, decode_object_position, encode_object_position);
/// The `onSetObjectRotation` descriptor.
pub const SET_OBJECT_ROTATION: IncomingRpc<ObjectRotation> =
    IncomingRpc::new(46, decode_object_rotation, encode_object_rotation);
/// The `onDestroyObject` descriptor.
pub const DESTROY_OBJECT: IncomingRpc<u16> = IncomingRpc::new(47, decode_u16, encode_u16);
/// The `onPlayerDeathNotification` descriptor.
pub const PLAYER_DEATH_NOTIFICATION: IncomingRpc<PlayerDeathNotification> = IncomingRpc::new(
    55,
    decode_player_death_notification,
    encode_player_death_notification,
);
/// The `onSetMapIcon` descriptor.
pub const SET_MAP_ICON: IncomingRpc<MapIcon> =
    IncomingRpc::new(56, decode_map_icon, encode_map_icon);
/// The `onRemoveVehicleComponent` descriptor.
pub const REMOVE_VEHICLE_COMPONENT: IncomingRpc<VehicleComponent> =
    IncomingRpc::new(57, decode_vehicle_component, encode_vehicle_component);
/// The `onRemove3DTextLabel` descriptor.
pub const REMOVE_3D_TEXT_LABEL: IncomingRpc<u16> = IncomingRpc::new(58, decode_u16, encode_u16);
/// The `onUpdateGlobalTimer` descriptor.
pub const UPDATE_GLOBAL_TIMER: IncomingRpc<i32> = IncomingRpc::new(60, decode_i32, encode_i32);
/// The `onDestroyPickup` descriptor.
pub const DESTROY_PICKUP: IncomingRpc<i32> = IncomingRpc::new(63, decode_i32, encode_i32);
/// The `onLinkVehicleToInterior` descriptor.
pub const LINK_VEHICLE_TO_INTERIOR: IncomingRpc<VehicleInterior> =
    IncomingRpc::new(65, decode_vehicle_interior, encode_vehicle_interior);
/// The `onSetPlayerColor` descriptor.
pub const SET_PLAYER_COLOR: IncomingRpc<PlayerColor> =
    IncomingRpc::new(72, decode_player_color, encode_player_color);
/// The `onRequestSpawnResponse` descriptor.
pub const REQUEST_SPAWN_RESPONSE: IncomingRpc<bool> =
    IncomingRpc::new(129, decode_bool8, encode_bool8);
/// The `onSetShopName` descriptor. The protocol field is exactly 32 bytes.
pub const SET_SHOP_NAME: IncomingRpc<[u8; 32]> =
    IncomingRpc::new(33, decode_fixed_string32, encode_fixed_string32);
/// The `onSetPlayerSkillLevel` descriptor.
pub const SET_PLAYER_SKILL_LEVEL: IncomingRpc<PlayerSkill> =
    IncomingRpc::new(34, decode_player_skill, encode_player_skill);
/// The `onRemoveBuilding` descriptor.
pub const REMOVE_BUILDING: IncomingRpc<RemoveBuilding> =
    IncomingRpc::new(43, decode_remove_building, encode_remove_building);
/// The `onAttachObjectToPlayer` descriptor.
pub const ATTACH_OBJECT_TO_PLAYER: IncomingRpc<AttachObjectToPlayer> = IncomingRpc::new(
    75,
    decode_attach_object_to_player,
    encode_attach_object_to_player,
);
/// The `onShowMenu` descriptor.
pub const SHOW_MENU: IncomingRpc<u8> = IncomingRpc::new(77, decode_u8, encode_u8);
/// The `onHideMenu` descriptor.
pub const HIDE_MENU: IncomingRpc<u8> = IncomingRpc::new(78, decode_u8, encode_u8);
/// The `onCreateExplosion` descriptor.
pub const CREATE_EXPLOSION: IncomingRpc<Explosion> =
    IncomingRpc::new(79, decode_explosion, encode_explosion);
/// The `onShowPlayerNameTag` descriptor.
pub const SHOW_PLAYER_NAME_TAG: IncomingRpc<PlayerNameTag> =
    IncomingRpc::new(80, decode_player_name_tag, encode_player_name_tag);
