...
unsafe extern "system" fn test_event_id(event: *const SampClientSdkEventV1) -> u8 {
    unsafe { (&*event.cast::<TestEvent>()).id }
}

unsafe extern "system" fn test_event_reset_read(
...
unsafe extern "system" fn test_event_clear(
    event: *mut SampClientSdkEventV1,
) -> SampClientSdkResult {
...
unsafe extern "system" fn test_event_remaining_bits(event: *mut SampClientSdkEventV1) -> usize {
    let event = unsafe { test_event(event) };
