...
impl HostApi {
    ...
    pub fn submit_force_onfoot_sync(self) -> Result<CommandReceipt<()>, SampClientSdkResult> {
        let mut receipt = SampClientSdkCommandReceipt::default();
        let result = unsafe { (self.raw.submit_force_onfoot_sync)(&mut receipt) };
        self.command_receipt(result, receipt)
    }
    /// Queues the documented R1 stats synchronization send.
    pub fn submit_force_stats_sync(self) -> Result<CommandReceipt<()>, SampClientSdkResult> {
        let mut receipt = SampClientSdkCommandReceipt::default();
    ...
    pub fn submit_force_passenger_sync(
        ...
        let mut receipt = SampClientSdkCommandReceipt::default();
        let result = unsafe { (self.raw.submit_force_passenger_sync)(vehicle, seat, &mut receipt) };
        self.command_receipt(result, receipt)
    }
