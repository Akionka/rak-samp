...
impl HostApi {
    ...
    /// return [`SampClientSdkResult::NotReady`] while the game-thread pump refreshes
    ...
    /// initially return [`SampClientSdkResult::NotReady`].
    ...
    ///
    /// The first lookup returns [`SampClientSdkResult::NotReady`] while the verified
    ...
    /// [`SampClientSdkResult::NotReady`] while the game-thread pump copies the flag.
    ...
    ///
    /// The first lookup returns [`SampClientSdkResult::NotReady`] while the
    /// verified game-thread pump copies the bounded native record. This never
    /// exposes a native chat or UI pointer.
    ...
    /// Returns one coherent owned snapshot of the active local R1 dialog.
    ///
