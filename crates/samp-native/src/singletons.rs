...
use super::{
    ...
    profile::{NativeProfile, NativeRva},
};
use std::ffi::c_void;

impl NativeProfile {
    pub fn singleton(self, rva: NativeRva, minimum_size: usize) -> Option<*mut c_void> {
    ...
    pub fn dialog_close_target(self) -> Option<usize> {
        self.module_base
            .checked_add(self.spec.ui.dialog.close_rva.get())
    }

    pub fn net_game_with_range(self, minimum_size: usize) -> Option<*mut c_void> {
        self.singleton(self.spec.net_game.singleton_rva, minimum_size)
    }

    pub fn dialog(self) -> Option<*mut c_void> {
        self.singleton(
    ...
    pub fn chat(self) -> Option<*mut c_void> {
        self.singleton(self.spec.ui.chat.singleton_rva, 1)
    }

    pub fn input(self) -> Option<*mut c_void> {
        self.singleton(
    ...
    pub fn scoreboard(self) -> Option<*mut c_void> {
        self.singleton(
    ...
    pub fn death_window(self) -> Option<*mut c_void> {
        self.singleton(self.spec.ui.death_window.singleton_rva?, 1)
    }

    pub fn game(self) -> Option<*mut c_void> {
        self.singleton(
    ...
    pub fn dialog_is_ready(self) -> bool {
        self.dialog().is_some()
    }

    pub fn chat_is_ready(self) -> bool {
        self.chat().is_some()
    }

    pub fn input_is_ready(self) -> bool {
        self.input().is_some()
    }

    pub fn scoreboard_is_ready(self) -> bool {
        self.scoreboard().is_some()
    }

    pub fn death_window_is_ready(self) -> bool {
        self.death_window().is_some()
    }

    pub fn game_is_ready(self) -> bool {
        self.game().is_some()
    }
}
...
mod tests {
    ...
    fn readiness_uses_the_profile_singletons() {
        for version in [
            ...
            let mut module = vec![0_u8; 0x2A_CA_24 + std::mem::size_of::<usize>()];
            let profile =
                NativeProfile::select(module.as_mut_ptr() as usize, version, version.entry_point())
                    .unwrap();
