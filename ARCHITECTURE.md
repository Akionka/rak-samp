...
| Native backend | `src/platform/win32/mod.rs`, `src/platform/win32/commands.rs`, `src/platform/win32/requests.rs`, `src/platform/win32/reads.rs`, `src/platform/win32/handles.rs`, `src/platform/win32/players.rs`, `src/platform/win32/text_labels.rs`, `src/platform/win32/r1_client.rs`, `src/platform/win32/r1_client/addresses.rs`, `src/platform/win32/r1_client/memory.rs`, `src/client.rs` | Select networking offsets, own hooks, bound producer-side game/cache requests, expose published scalar, snapshot, animation-catalog, handle-cache, player, and text-label reads, isolate approved R1/GTA addresses and native layouts, and validate every direct access through guarded memory helpers. |
