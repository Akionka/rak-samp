...
| Native backend | `src/platform/win32/mod.rs`, `src/platform/win32/chat_entries.rs`, `src/platform/win32/commands.rs`, `src/platform/win32/requests.rs`, `src/platform/win32/reads.rs`, `src/platform/win32/gangzones.rs`, `src/platform/win32/handles.rs`, `src/platform/win32/hooks.rs`, `src/platform/win32/objects.rs`, `src/platform/win32/packets.rs`, `src/platform/win32/players.rs`, `src/platform/win32/text_labels.rs`, `src/platform/win32/textdraws.rs`, `src/platform/win32/vehicles.rs`, `src/platform/win32/r1_client.rs`, `src/platform/win32/r1_client/addresses.rs`, `src/platform/win32/r1_client/memory.rs`, `src/client.rs` | Select networking offsets, own hooks, validate and dispatch native bitstreams/raw packets through listeners, perform native outbound sends and incoming packet/RPC emulation, bound producer-side game/cache requests, expose published scalar, snapshot, animation-catalog, chat-history, gangzone, handle-cache, object, player, text-label, textdraw, and vehicle reads, isolate approved R1/GTA addresses and native layouts, and validate every direct access through guarded memory helpers. |
