...
`0x7FFF` also proves the public ready/version APIs and the SDK's opaque module
...
verifies that `Samp::chat().display_mode()` returns one of SA-MP's documented
display modes. The probe then waits for an operator
