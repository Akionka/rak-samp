# DL-R1 full live validation probe

This opt-in ASI validates the DL-R1 host profile only against the disposable
...
restoration, and packet/RPC delivery after reconnect. Public DL unsafe/raw
addresses are outside this probe because that API remains intentionally R1-only.

## Build and install

1. Close the isolated DL GTA client.
2. Build the host and probe from the repository root:

   ```powershell
   cargo build --release -p samp-client-sdk-host
   cargo build --release -p samp-client-sdk-dl-network-probe
   ```

3. Copy these files into the isolated DL GTA directory and use the `.asi`
   extension:

   ```text
   D:\cargo-build\i686-pc-windows-msvc\release\samp_client_sdk.dll
   D:\cargo-build\i686-pc-windows-msvc\release\samp_client_sdk_dl_network_probe.dll
   ```

4. Compile `server\dl_network_probe.pwn` and
   `server\npcmodes\dl_probe_bot.pwn` with the server package's `pawncc`.
...
1. Start one DL client and connect to `127.0.0.1:7777`. The fixture launcher
   starts and moves `DLProbeBot` automatically.
...
3. Do not interfere while the probe opens and closes dialogs and moves the
   ...
   `DL_SDK_INCOMING_20260812` message and the local UI/death messages must be
   visible.
...
5. Type `/dlsdkreconnect`. The probe disconnects and
...
The ASI writes `samp-client-sdk-dl-network-probe.status` in GTA's working
directory. The server console must contain `DL_OUTBOUND_OK`,
`DL_INCOMING_SENT`, and `DL_ENTITIES_SENT`. Vehicle transitions use hidden
...
`SampClientSdkDLNetworkProbe_Shutdown` from its worker thread before
