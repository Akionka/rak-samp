//! Loopback-only DL-R1 network delivery validation.
//!
...
compile_error!("samp_client_sdk_dl_network_probe supports only 32-bit Windows x86 targets");
...
const OUTBOUND_MARKER: &[u8] = b"DL_SDK_OUTBOUND_20260812";
const INCOMING_MARKER: &[u8] = b"DL_SDK_INCOMING_20260812";
const DIALOG_REQUEST_MARKER: &[u8] = b"DL_SDK_DIALOG_REQUEST_20260812";
const ENTITY_REQUEST_MARKER: &[u8] = b"DL_SDK_ENTITY_REQUEST_20260813";
const ENTITY_IDS_PREFIX: &[u8] = b"DL_SDK_ENTITY_IDS_";
const CHAT_INPUT_TEXT_MARKER: &[u8] = b"DL_SDK_TEXT_CACHE_20260812";
const LOCAL_CHAT_MARKER: &[u8] = b"DL SDK full UI validation";
const LOCAL_COMMAND_NAME: &[u8] = b"dlsdkprobe";
const LOCAL_COMMAND_TEXT: &[u8] = b"/dlsdkprobe consolidated";
const LOCAL_DIALOG_INPUT_TEXT: &[u8] = b"DL_INPUT_UPDATED";
const LOCAL_LABEL_TEXT: &[u8] = b"DL label validation";
const LOCAL_LABEL_UPDATED_TEXT: &[u8] = b"DL label updated";
const LOCAL_TEXTDRAW_TEXT: &[u8] = b"DL textdraw validation";
const LOCAL_TEXTDRAW_UPDATED_TEXT: &[u8] = b"DL textdraw updated";
const LOCAL_DRIVER_REQUEST: &[u8] = b"DL_SDK_LOCAL_DRIVER_REQUEST";
const LOCAL_PASSENGER_REQUEST: &[u8] = b"DL_SDK_LOCAL_PASSENGER_REQUEST";
const LOCAL_TRAILER_REQUEST: &[u8] = b"DL_SDK_LOCAL_TRAILER_REQUEST";
const VEHICLE_CLEANUP_REQUEST: &[u8] = b"DL_SDK_VEHICLE_CLEANUP";
const RECONNECT_COMMAND_NAME: &[u8] = b"dlsdkreconnect";
...
const STATUS_FILE: &str = "samp-client-sdk-dl-network-probe.status";
const DL_R1_ENTRY_POINT_RVA: u32 = 0x0FDB60;
const DL_CONNECTED_STATE: i32 = 5;
const DL_WAIT_JOIN_STATE: i32 = 6;
const MAX_PE_HEADER_OFFSET: usize = 0x1000;

/// The SDK host API resolved for this probe.
pub const STATUS_HOST_CONNECTED: u32 = 1 << 0;
/// Public host status, version, and the opaque module base identify the pinned DL-R1 image.
pub const STATUS_RUNTIME_IDENTITY: u32 = 1 << 1;
/// The DL CNetGame scalar cache reported the expected connected server values.
pub const STATUS_CNETGAME_SCALARS: u32 = 1 << 6;
/// The DL cached local-player snapshot passed bounded sanity checks.
pub const STATUS_LOCAL_PLAYER_SNAPSHOT: u32 = 1 << 7;
/// The DL cached player-pool count pair and largest ID matched the loopback session.
pub const STATUS_PLAYER_POOL_SCALARS: u32 = 1 << 12;
/// The DL cached scoreboard flag observed both the open and closed states.
pub const STATUS_SCOREBOARD_CACHE: u32 = 1 << 13;
/// The DL cached chat display mode returned a documented native value.
pub const STATUS_CHAT_DISPLAY_MODE: u32 = 1 << 14;
/// The DL cached cursor mode returned a documented native value.
pub const STATUS_CURSOR_MODE: u32 = 1 << 15;
/// The DL cached remote-player directory observed another loopback client.
pub const STATUS_REMOTE_PLAYER_DIRECTORY: u32 = 1 << 16;
/// DL object, pickup, vehicle, gangzone, and player-ped handle paths round-tripped.
pub const STATUS_ENTITY_HANDLES: u32 = 1 << 17;
/// DL native local-player force-sync methods completed on the game thread.
pub const STATUS_FORCE_SYNC_RECEIPTS: u32 = 1 << 18;
/// DL chat, input, cursor, scoreboard, and death-window mutations completed.
pub const STATUS_UI_MUTATIONS: u32 = 1 << 19;
/// DL client-side input/list dialogs and the close-response hook passed.
pub const STATUS_DIALOG_LIFECYCLE: u32 = 1 << 20;
/// An DL native chat command was installed, invoked, and removed synchronously.
pub const STATUS_CHAT_COMMAND_LIFECYCLE: u32 = 1 << 21;
/// The DL animation table round-tripped one name/file pair.
pub const STATUS_ANIMATION_TABLE: u32 = 1 << 22;
/// DL local-player colour/action and all send-rate writes completed and restored.
pub const STATUS_LOCAL_MUTATIONS: u32 = 1 << 23;
/// An automatically allocated DL text label completed create/read/write/delete.
pub const STATUS_TEXT_LABEL_LIFECYCLE: u32 = 1 << 24;
/// A free DL textdraw slot completed create/read/write/delete.
...
/// Disconnect invalidated the DL connection-bound caches.
pub const STATUS_DISCONNECT_INVALIDATION: u32 = 1 << 28;
/// Reconnect restored DL caches and post-reconnect packet/RPC delivery.
...
/// The DL cached chat-input active flag and command names passed the live check.
pub const STATUS_CHAT_INPUT_CACHE: u32 = 1 << 8;
/// The DL cached dialog active flag observed the disposable server dialog.
...
/// The DL cached chat-input text matched the operator-entered marker.
        ...
        DLL_PROCESS_ATTACH => {
            ...
            *VEHICLE_PHASES
            ...
            if thread::Builder::new()
                .name("samp-client-sdk-dl-network-probe-init".into())
                .spawn(initialize)
                .is_err()
            {
...
fn record_vehicle_phase(message: &[u8]) {
    let mut phases = VEHICLE_PHASES
        .lock()
        .unwrap_or_else(|error| error.into_inner());
    if let Some(values) = parse_u16_fields(message, b"DL_SDK_LOCAL_DRIVER_READY_") {
        phases.local_driver = (values.len() == 1).then_some(values[0]);
    } else if let Some(values) = parse_u16_fields(message, b"DL_SDK_LOCAL_PASSENGER_READY_") {
        phases.local_passenger = (values.len() == 1).then_some(values[0]);
    } else if let Some(values) = parse_u16_fields(message, b"DL_SDK_LOCAL_TRAILER_READY_") {
        phases.local_trailer = (values.len() == 2).then(|| VehiclePair {
            vehicle: values[0],
            trailer: values[1],
        });
    } else if message == b"DL_SDK_VEHICLE_CLEANUP_READY" {
        phases.cleanup = true;
    }
}
...
fn verify_runtime_identity(samp: Samp) -> Result<(), SampClientSdkResult> {
    ...
    if samp.version()? != SampClientSdkClientVersion::Dl {
        return Err(SampClientSdkResult::NativeCallFailed);
    }
    let Some(base) = (unsafe { raw::base() }) else {
        return Err(SampClientSdkResult::NotReady);
    };
    if unsafe { module_entry_point_rva(base.as_ptr()) } != Some(DL_R1_ENTRY_POINT_RVA) {
        return Err(SampClientSdkResult::NativeCallFailed);
    }
    Ok(())
}
    ...
    loop {
        ...
        match (samp.game_state(), samp.server().info()) {
            (Ok(game_state), Ok(info)) => {
                ...
                if game_state == DL_WAIT_JOIN_STATE
...
fn verify_cached_chat_input(samp: Samp) -> Result<(), SampClientSdkResult> {
    ...
    loop {
        ...
        match (
            ...
            input.is_command_defined(b"dl_sdk_probe_missing_command"),
        ) {
...
fn verify_ui_mutations(samp: Samp) -> Result<(), SampClientSdkResult> {
    ...
    wait_for_receipt(chat.set_entry(99, LOCAL_CHAT_MARKER, b"DL SDK", 0xFF6FCF97, 0xFFFFFFFF)?)?;
    wait_for_condition(SCALAR_CACHE_TIMEOUT, || {
        chat.entry(99).map(|entry| {
            entry.text == LOCAL_CHAT_MARKER
                && entry.prefix == b"DL SDK"
    ...
    let input = samp.chat_input();
    wait_for_receipt(input.set_text(b"DL_SDK_MUTATION")?)?;
    wait_for_condition(CHAT_INPUT_CACHE_TIMEOUT, || {
        input.text().map(|text| text == b"DL_SDK_MUTATION")
    })?;
...
fn verify_dialog_lifecycle(samp: Samp) -> Result<(), SampClientSdkResult> {
    ...
    wait_for_receipt(dialogs.show(LocalDialog {
        ...
        title: b"DL input dialog",
        text: b"DL input body",
        button1: b"Accept",
        button2: b"Cancel",
    })?)?;
        ...
        dialogs.active().map(|dialog| {
            dialog.is_some_and(|dialog| {
                dialog.id == 26_000
                    ...
                    && dialog.title == b"DL input dialog"
                    && dialog.text == b"DL input body"
                    && !dialog.server_side
            })
        })
    })?;
...
fn verify_reconnect_on_request(samp: Samp) -> Result<(), SampClientSdkResult> {
    ...
    wait_for_receipt(samp.chat().add(LocalChatMessage {
        style: LocalChatMessageStyle::Info,
        text: b"DL main pass complete. Type /dlsdkreconnect for the final lifecycle pass.",
        prefix: b"",
        text_colour: 0xFF6FCF97,
        prefix_colour: 0,
    })?)?;
    ...
    loop {
        ...
        if server_not_ready && local_not_ready && !samp.net().incoming_emulation_ready() {
            break;
        }
        if deadline.saturating_duration_since(Instant::now()).is_zero() {
            return Err(SampClientSdkResult::TimedOut);
    ...
    wait_for_condition(HOST_CONNECTION_TIMEOUT, || {
        ...
        let ready = observation.server_ready
            && observation.game_state == Some(DL_CONNECTED_STATE)
...
mod tests {
    ...
    #[test]
    fn parses_the_dl_entry_point_from_a_bounded_pe_header() {
        ...
        image[0xA8..0xAC].copy_from_slice(&DL_R1_ENTRY_POINT_RVA.to_le_bytes());
    ...
    fn parses_the_server_owned_entity_id_marker() {
        let ids = parse_entity_ids(b"DL_SDK_ENTITY_IDS_17,18,19,20").unwrap();
        assert_eq!(ids.object, 17);
        assert_eq!(ids.vehicle, 18);
        assert_eq!(ids.pickup, 19);
        assert_eq!(ids.gangzone, 20);
        assert!(parse_entity_ids(b"DL_SDK_ENTITY_IDS_17,18,19").is_none());
        assert!(parse_entity_ids(b"DL_SDK_ENTITY_IDS_17,18,19,20,21").is_none());
    }
    ...
    fn parses_bounded_vehicle_phase_fields() {
        assert_eq!(
            parse_u16_fields(
                b"DL_SDK_REMOTE_TRAILER_READY_7,18,19",
                b"DL_SDK_REMOTE_TRAILER_READY_"
            ),
            Some(vec![7, 18, 19])
        );
        assert!(
            parse_u16_fields(
                b"DL_SDK_REMOTE_TRAILER_READY_7,nope,19",
                b"DL_SDK_REMOTE_TRAILER_READY_"
            )
            .is_none()
        );
