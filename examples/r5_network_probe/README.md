# R5-1 full live validation probe

This opt-in ASI validates the R5-1 host profile only against the disposable
...
restoration, and packet/RPC delivery after reconnect. Public R5 unsafe/raw
addresses are outside this probe because that API remains intentionally R1-only.

## Build and install

1. Close the isolated R5 GTA client.
2. Build the host and probe from the repository root:

   ```powershell
   cargo build --release -p samp-client-sdk-host
   cargo build --release -p samp-client-sdk-r5-network-probe
   ```

3. Copy these files into the isolated R5 GTA directory and use the `.asi`
   extension:

   ```text
   D:\cargo-build\i686-pc-windows-msvc\release\samp_client_sdk.dll
   D:\cargo-build\i686-pc-windows-msvc\release\samp_client_sdk_r5_network_probe.dll
   ```

4. Compile `server\r5_network_probe.pwn` and
   `server\npcmodes\r5_probe_bot.pwn` with the server package's `pawncc`.
...
1. Start one R5 client and connect to `127.0.0.1:7777`. The fixture launcher
   starts and moves `R5ProbeBot` automatically.
...
3. Do not interfere while the probe opens and closes dialogs and moves the
   ...
   `R5_SDK_INCOMING_20260812` message and the local UI/death messages must be
   visible.
...
5. Type `/r5sdkreconnect`. The probe disconnects and
...
The ASI writes `samp-client-sdk-r5-network-probe.status` in GTA's working
directory. The server console must contain `R5_OUTBOUND_OK`,
`R5_INCOMING_SENT`, and `R5_ENTITIES_SENT`. Vehicle transitions use hidden
...
`SampClientSdkR5NetworkProbe_Shutdown` from its worker thread before
