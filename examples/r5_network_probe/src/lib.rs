//! Loopback-only R5-1 network delivery validation.
//!
...
const DIALOG_REQUEST_MARKER: &[u8] = b"R5_SDK_DIALOG_REQUEST_20260812";
const ENTITY_REQUEST_MARKER: &[u8] = b"R5_SDK_ENTITY_REQUEST_20260813";
const ENTITY_IDS_PREFIX: &[u8] = b"R5_SDK_ENTITY_IDS_";
const CHAT_INPUT_TEXT_MARKER: &[u8] = b"R5_SDK_TEXT_CACHE_20260812";
const LOCAL_CHAT_MARKER: &[u8] = b"R5 SDK full UI validation";
const LOCAL_COMMAND_NAME: &[u8] = b"r5sdkprobe";
const LOCAL_COMMAND_TEXT: &[u8] = b"/r5sdkprobe consolidated";
const LOCAL_DIALOG_INPUT_TEXT: &[u8] = b"R5_INPUT_UPDATED";
const LOCAL_LABEL_TEXT: &[u8] = b"R5 label validation";
const LOCAL_LABEL_UPDATED_TEXT: &[u8] = b"R5 label updated";
const LOCAL_TEXTDRAW_TEXT: &[u8] = b"R5 textdraw validation";
const LOCAL_TEXTDRAW_UPDATED_TEXT: &[u8] = b"R5 textdraw updated";
const LOCAL_DRIVER_REQUEST: &[u8] = b"R5_SDK_LOCAL_DRIVER_REQUEST";
const LOCAL_PASSENGER_REQUEST: &[u8] = b"R5_SDK_LOCAL_PASSENGER_REQUEST";
const LOCAL_TRAILER_REQUEST: &[u8] = b"R5_SDK_LOCAL_TRAILER_REQUEST";
const VEHICLE_CLEANUP_REQUEST: &[u8] = b"R5_SDK_VEHICLE_CLEANUP";
const RECONNECT_COMMAND_NAME: &[u8] = b"r5sdkreconnect";
...
const R5_1_ENTRY_POINT_RVA: u32 = 0x0CBC90;
const R5_CONNECTED_STATE: i32 = 5;
const R5_WAIT_JOIN_STATE: i32 = 6;
...
fn record_vehicle_phase(message: &[u8]) {
    let mut phases = VEHICLE_PHASES
        .lock()
        .unwrap_or_else(|error| error.into_inner());
    if let Some(values) = parse_u16_fields(message, b"R5_SDK_LOCAL_DRIVER_READY_") {
        phases.local_driver = (values.len() == 1).then_some(values[0]);
    } else if let Some(values) = parse_u16_fields(message, b"R5_SDK_LOCAL_PASSENGER_READY_") {
        phases.local_passenger = (values.len() == 1).then_some(values[0]);
    } else if let Some(values) = parse_u16_fields(message, b"R5_SDK_LOCAL_TRAILER_READY_") {
        phases.local_trailer = (values.len() == 2).then(|| VehiclePair {
            vehicle: values[0],
            trailer: values[1],
        });
    } else if message == b"R5_SDK_VEHICLE_CLEANUP_READY" {
        phases.cleanup = true;
    }
}
...
fn verify_runtime_identity(samp: Samp) -> Result<(), SampClientSdkResult> {
    ...
    if samp.version()? != SampClientSdkClientVersion::R5_1 {
        return Err(SampClientSdkResult::NativeCallFailed);
    }
    let Some(base) = (unsafe { raw::base() }) else {
        return Err(SampClientSdkResult::NotReady);
    };
    if unsafe { module_entry_point_rva(base.as_ptr()) } != Some(R5_1_ENTRY_POINT_RVA) {
        return Err(SampClientSdkResult::NativeCallFailed);
    }
    Ok(())
}
    ...
    loop {
        ...
        match (samp.game_state(), samp.server().info()) {
            (Ok(game_state), Ok(info)) => {
                ...
                if game_state == R5_WAIT_JOIN_STATE
...
fn verify_reconnect_on_request(samp: Samp) -> Result<(), SampClientSdkResult> {
    ...
    wait_for_condition(HOST_CONNECTION_TIMEOUT, || {
        ...
        let ready = observation.server_ready
            && observation.game_state == Some(R5_CONNECTED_STATE)
...
mod tests {
    ...
    fn parses_the_r5_entry_point_from_a_bounded_pe_header() {
        ...
        image[0xA8..0xAC].copy_from_slice(&R5_1_ENTRY_POINT_RVA.to_le_bytes());
    ...
    fn parses_the_server_owned_entity_id_marker() {
        let ids = parse_entity_ids(b"R5_SDK_ENTITY_IDS_17,18,19,20").unwrap();
        assert_eq!(ids.object, 17);
        assert_eq!(ids.vehicle, 18);
        assert_eq!(ids.pickup, 19);
        assert_eq!(ids.gangzone, 20);
        assert!(parse_entity_ids(b"R5_SDK_ENTITY_IDS_17,18,19").is_none());
        assert!(parse_entity_ids(b"R5_SDK_ENTITY_IDS_17,18,19,20,21").is_none());
    }
