...
unsafe extern "system" fn test_local_chat_display_mode(output: *mut i32) -> RakSampResult {
    let Some(output) = (unsafe { output.as_mut() }) else {
        return RakSampResult::InvalidArgument;
    };
    *output = 2;
...
unsafe extern "system" fn test_local_dialog_active(output: *mut u8) -> RakSampResult {
    let Some(output) = (unsafe { output.as_mut() }) else {
        return RakSampResult::InvalidArgument;
    };
    *output = 0;
...
unsafe extern "system" fn test_local_chat_input_active(output: *mut u8) -> RakSampResult {
    let Some(output) = (unsafe { output.as_mut() }) else {
        return RakSampResult::InvalidArgument;
    };
    *output = 0;
...
unsafe extern "system" fn test_player_count(include_npcs: u8, output: *mut u16) -> RakSampResult {
    let Some(output) = (unsafe { output.as_mut() }) else {
        return RakSampResult::InvalidArgument;
    };
    match include_npcs {
...
unsafe extern "system" fn test_vehicle_exists(id: u16, output: *mut u8) -> RakSampResult {
    let Some(output) = (unsafe { output.as_mut() }) else {
        return RakSampResult::InvalidArgument;
    };
    *output = u8::from(id == 7);
...
unsafe extern "system" fn test_textdraw_exists(id: u16, output: *mut u8) -> RakSampResult {
    let Some(output) = (unsafe { output.as_mut() }) else {
        return RakSampResult::InvalidArgument;
    };
    *output = u8::from(id == 7);
